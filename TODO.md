...
Status: **v1.2.3**. External config engine + adaptive `done` gate + per-session marker.
Configuration via `~/.claude/sound-alerts.local.conf` (enabled / volume / doneMode /
doneMinSeconds + per-sound `disabled=`) with robust defaults in the script. Distributed from the
public GitHub repo `furesipaolo-design/claude-sound-alerts` (MIT-licensed code). Test harness:
`scripts/test-engine.sh` (14). Pool: **42 sounds** (13 permission · 13 question · 16 done), all
peak-normalized to −1 dBFS. Cross-platform player (afplay / ffplay / mpv / mpg123).
